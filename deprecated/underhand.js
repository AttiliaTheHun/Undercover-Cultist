...
* THIS CODE CONTAINS KNOWN BUGS AND BAD PROGRAMMING APPROACHES,
* THIS FILE WAS REPLACED WITH /commands/play.js
*/
  ...
  async execute(msg, args, client) {
    try {
      ...
      function new_game() {
        ...
        for (let i = 0; i < 9; i++) {
          const random = Math.floor(Math.random() * randoms.length);
      ...
      while (run) {
        ...
        for (let i = 0; i < 3; i++) { //indicates option number
          ...
          if (consarr.findIndex((num) => {
            ...
            const index = consarr.findIndex((num) => {
              if (num == 420) {
                return true;
              }
              return false;
        ...
        await message.channel.awaitMessages(filter, { time: 180 * 1000, max: 1, errors: ["time"] })
          .then(messages => { //when message sent
            ...
            if (event[`option${(optsel)}`].iswin != "") {
              const god = event[`option${(optsel)}`].iswin;
              message.channel.send("**You Won**", {
              ...
              if (i == 0) { //you don't wanna double up player's relics
                ...
                if (consarr[i] > resources[i] + resources[0]) { //check if enough resources
                  ...
                  if (consarr[i] > resources[i]) { //check if relics needed
                    const overflow = consarr[i] - resources[i]; //get how many relics needed
            ...
            const specificids = event[`option${(optsel)}`].shuffle.specificids;
            ...
            if (event[`option${(optsel)}`].outputtext != null) {
              if (specificids[0] == null && event[`option${(optsel)}`].outputtext.includes("Insert")) {
                ...
                const type = event[`option${(optsel)}`].outputtext.substring(event[`option${(optsel)}`].outputtext.indexOf("'") + 1, event[`option${(optsel)}`].outputtext.lastIndexOf("'")).toLowerCase();
                const count = event[`option${(optsel)}`].shuffle.numcards;
            ...
            const ranreq = event[`option${(optsel)}`].randomrequirements; //get the random requirements
            ...
            for (let f = 0; f < ranreq; f++) {
              ...
              const random = Math.floor(Math.random() * indexes.length); //get random number applyable to the indexes array as index
