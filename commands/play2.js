  ...
  async handleUserInput({handlingForesightDiscard}){
    ...
    class Event {
      ...
      async sendCardTexture(cardNumber){
        ...
        await message.channel.send("", {
            files: [`http://underhand.clanweb.eu/res/Card${cardNumber}.png`]
          });
      }
    ...
    */
    class Option {
      constructor({game, optionData}) {
        ...
        this.hasRandomRequirements = (optionData.shuffle.numcards > 0),
        this.randomRequirements = optionData.shuffle.numcards > 0,
        ...
        for(let resource_type = 0; resource_type < this.consume_values.length; resource_type++){
          if(this.consume_values[resource_type] == 420 && this.game.player.resources[resource_type] > 1){
    ...
    class Cheats {
      ...
      doAction(actionData){
        ...
        switch(actionData){
          ...
          default:
            if(actionData > 6){
              this.game.player.resources[actionData] += 100;
            }
        }
        this.cheated();
      }
    ...
    */
    class Mysc {
      ...
      /**
      * Add discard_deck to the base_deck and then shuffle base_deck to prepare
      * a new round of the game
      */
      async reshuffle(){
        this.game.base_deck.push(this.game.discard_deck);
        this.game.discard_deck = [];
        this.game.base_deck = utils.shuffleArray(this.game.base_deck);
