  ...
  async execute(message, args, client) { 	
    ...
    return;
       ...
       */
        ...
        if(event[option_selected].iswin != ""){
          let god = event[option_selected].iswin;
    ...
    if(event[option_selected].foresight.hasforesight == 1){
      if(base_deck.length < 4){
        let placeholder_deck = base_deck;
...
/*
 let initial_events = [];
    for(let i = 1; i < 118; i++){
     ...
     for(let i = 0; i < 9; i++){
      let random = Math.floor(Math.random() * initial_events.length);
  ...
  let total_resources = module.exports.summarizeArray(resources);
